        ...
        if ($action === 'create') {
            ...
            if ($assignee && $assignee !== (int)$user['id']) {
                notifyUser(
                    ...
                    'task_assigned',
                    'New task assigned',
                    "{$user['name']} assigned you \"{$title}\".",
                    "views/tasks.php?team={$teamId}",
                    (int)$user['id'],
                    $teamId,
                    $taskId
                );
            ...
            $st = $db->prepare("SELECT creator_id,team_id,status,assignee_id,title FROM tasks WHERE id=?"); $st->execute([$taskId]);
            $task = $st->fetch();
            ...
            if ($task['status'] !== $status) {
                $oldStatus = $statusLabels[$task['status']] ?? $task['status'];
                $newStatus = $statusLabels[$status] ?? $status;
                notifyTeamMembers(
                    (int)$task['team_id'],
                    'task_status_changed',
                    'Task status changed',
                    "\"{$title}\" moved from {$oldStatus} to {$newStatus}.",
                    "views/tasks.php?team={$task['team_id']}",
                    (int)$user['id'],
                    $taskId,
                    [(int)$user['id']]
                );
            }
            if ($assignee && $assignee !== (int)($task['assignee_id'] ?? 0) && $assignee !== (int)$user['id']) {
                notifyUser(
                    $assignee,
                    'task_assigned',
