    ...
    markReadBtn.addEventListener('click', async (e) => {
      ...
      try {
        const res = await fetch('../api/notifications.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'mark_all_read', csrf: markReadBtn.dataset.csrf })
        });
        const data = await res.json();
        if (!data.success) throw new Error(data.error || 'Failed');

        document.querySelector('#notifDropdown .badge')?.remove();
        document.querySelectorAll('#notifDropdown + .dropdown-menu li:not(:first-child)').forEach(li => li.remove());
        const empty = document.createElement('li');
        empty.className = 'p-3 text-center text-muted small';
        empty.textContent = 'No new notifications';
        document.querySelector('#notifDropdown + .dropdown-menu')?.appendChild(empty);
        markReadBtn.remove();
